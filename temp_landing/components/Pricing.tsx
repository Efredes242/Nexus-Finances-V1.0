      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {PRICING.map((plan) => (
            <div 
              key={plan.name} 
              className={`relative glass rounded-[2.5rem] p-10 flex flex-col border transition-all hover:scale-[1.02] ${plan.recommended ? 'border-blue-500/50 bg-blue-600/5 shadow-2xl shadow-blue-900/10' : 'border-white/5'}`}
            >
              {plan.recommended && (
              ...
              <button className={`w-full py-4 rounded-2xl font-extrabold transition-all transform active:scale-95 ${plan.recommended ? 'bg-blue-600 text-white blue-glow hover:bg-blue-500' : 'bg-white/5 text-white hover:bg-white/10 hover:border-white/20 border border-white/5'}`}>
                Seleccionar Plan
              </button>
            </div>
          ))}
